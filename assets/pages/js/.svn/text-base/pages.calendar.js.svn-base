    ...
    $.fn.pagescalendar = function(method) {
        ...
        var helpers = {
            ...
            loadEvents: function() {
                ...
                //Data
                var customDataHtml = new Array();
                var hasBatch = 0;
                var thisDataHasData = 0;
                ...
                for (var i = 0; i < settings.events.length; i++) {
                    ...
                    var currentWeekStart = moment([calendar.year, calendar.month, calendar.date]).startOf('day').format("YYYY-MM-DD");
                    var eventWeekStart = moment(settings.events[i].start).startOf('day').format('YYYY-MM-DD');

                    if (currentWeekStart.localeCompare(eventWeekStart) == 0) {
                        // settings.events[i].title;
                        hasBatch = 1;
                        thisData = settings.events[i];
                        thisDataHasData = 1;
                    }
                    if(thisDataHasData == 1){
                        entireData.push(thisData);
                    }
                }

                console.log(entireData);

                var scope = angular.element(document.getElementById("batchListAngularController")).scope();
                    scope.$apply(function() {
                    scope.updateBatchesData(entireData);
                });

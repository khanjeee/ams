...
QUERY;
    ...
    function getAllCampaigns($aParams)
    {
        ...
        $sOrderBy = '';
        if ($returnCount) {
            $sSelect = ' COUNT(c.campaign_id) AS count ';
        }
        else
        {
            $sSelect = 'c.campaign_id,c.title,c.description,c.created_on';
            ...
            $sOrderBy = ' ORDER BY c.campaign_id DESC ';
        }
...
SQL;
    ...
    {
        $data = array('is_deleted' => '1');
        $this->db->where('campaign_id', $iCampaignId);
    ...
    public function loadCampaign($iCampaignId=1)
    {
        ...
        if($iCampaignId)
        {
            ...
            SELECT * FROM campaigns where campaign_id='$iCampaignId'
            LIMIT 1
SQL;
            ...
            if($aBatches)
            {
                ...
                {
                    $aCampaignData['aBatches'][$aBatches[$b]['campaign_batch_id']]                              =     $this->getBatchSummary($aBatches[$b]['campaign_batch_id']);
                    $aCampaignData['aBatches'][$aBatches[$b]['campaign_batch_id']]['campaign_batch_id']         =     $aBatches[$b]['campaign_batch_id'];
                }
            }
        }
        ...
        if($iCampaignId)
        {
            $SQL = <<<SQL
               ...
               SELECT
                ...
                FROM campaigns where campaign_id='$iCampaignId'
              LIMIT 1
SQL;
            $DbResult                           =   $this->db->query($SQL);
            return $DbResult->row();

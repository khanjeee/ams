...
class Predefined_Batch_Model extends CI_Model
{
    ...
    { 
		...
		UPDATE predefined_campaign_batches SET
                title='$sTitle',description='$sDesc',last_updated_on='$dDate',last_updated_by='$iUserId'
                WHERE predefined_campaign_batch_id='$iBatchId' LIMIT 1
SQL;
            ...
            else
            {
                ...
                INSERT INTO predefined_campaign_batches
                (
                    predefined_campaign_id,user_id,title,description,current_status,created_on,created_by
                )
                VALUES
                (
                  '$iCampaignId',
               ...
               if(!empty($aInnerValues))
                {
                   $sInnerValues = implode(',',$aInnerValues);
                    ...
                    INSERT INTO predefined_campaign_batches_lists
                      $sInnerKeys 
                     VALUES     
                      $sInnerValues
SQL;
...
SQL;
        ...
        return $SQL->row('predefined_template_content_id');
    }
    ...
    function ScheduleBatch($sCallFrom='',$aData = array())
    {
        if($aData)
        {
            ...
            $SQL = <<<SQL
                ...
                WHERE  predefined_campaign_batch_id  = '$iCampaignBatchId' LIMIT 1
SQL;
            if ($this->db->query($SQL)) {return $this->db->affected_rows();}
        }
        return 0;
    ...
    function getCutOffBatches()
    {
        ...
        ORDER BY predefined_campaign_batch_id DESC

SQL;
        $DbResult                           =   $this->db->query($SQL);
        return $DbResult->result_array('predefined_campaign_batch_id');
    }
...
SQL;
    ...
    function DownloadBatchContent($BatchId = 0)
    {
        if($BatchId)
        {
            ...
            AND  predefined_campaign_batch_id  = '$BatchId'
SQL;
        }
    ...
    function DownloadDefaultContent($BatchId = 0)
    {
        if($BatchId)
        {
            ...
            IN
            (
            SELECT `template_id` FROM `campaign_batches` WHERE `predefined_campaign_batch_id` = '$BatchId'
            )
            )
SQL;
        }
...
SQL;
    ...
    function BatchInfo($iBatchId=0)
    {
        if($iBatchId)
        {
            ...
            FROM     predefined_campaign_batches cb
            ...
            LEFT JOIN         predefined_campaigns  pc  ON pc.predefined_campaign_id = cb.predefined_campaign_id


            WHERE             cb.predefined_campaign_batch_id  =   '$iBatchId'
    ...
    function getCutOffPeriod($CampaignBatchId)
    {
        if($CampaignBatchId)
        {
            ...
            FROM templates WHERE template_id=( SELECT
              template_id
            FROM predefined_campaign_batches WHERE predefined_campaign_batch_id ='$CampaignBatchId')
    ...
    function createBatchLog($aData = array())
    {
      if($aData)
      {
          //updating status in campaign batches table
            ...
            $this->db->where('predefined_campaign_batch_id', $aData['predefined_campaign_batch_id']);
         ...
         //insertion of log in campaign logs table
            $data = array(
                        'predefined_campaign_batch_id' => $aData['predefined_campaign_batch_id'] ,
                        'status'        =>  $aData['status'] ,
...
SQL;
    ...
    {
         $this->db->select('predefined_campaign_batch_id');       

...
class Users extends CI_Model {
    ...
    function emailAlreadyExist($email='')
    {
      ...
      function addSubscriber($aData = array())
    { 
        if ($aData)
        {
            ...
            $sAddressBilling    = $aData['inserted_values']['billing'];
				...
				INSERT INTO  users 
						(
						role_id, first_name, last_name, email, password,gender,address_mailing,address_billing,                                                        is_active,joined_on)
				 VALUES (
                            '$iRoleId', '$sFirstName', '$sLastName', '$sEmail', '$spassword','$sGender','$sAddressMailing','$sAddressBilling', 
                             '$iIs_active','$dDate' )
...
SQL;
    ...
    private function _defaultUserLogin($sType = "", $aData = array()) {
        if ($aData) {
            ...
            $SQL = <<<SQL
                SELECT user_id ,role_id,first_name,last_name,package_id,email FROM users WHERE  $sLoginKey = '$sLoginValue' AND  $sLoginSecret = '$sSecretValue' AND is_active = 1  LIMIT 1
SQL;
    ...
    /*
     * This function will update User Profiles
     */

    private function _saveUser($aData = array())
    { 
        if ($aData)
        {
            $iUserId            = $aData['user_id'];
            ...
            $iIs_active         = $aData['is_active'];
            $sGender            = $aData['gender'];
            $dDate              = date(DATE_FORMAT_MYSQL);
            $LoggedInUser       = getLoggedInUserId();

            if ($iUserId) { // need to update!
                if ($aData['password']) {
                    $sQueryChangePassword = " password  =  '" . $spassword . "', ";
                }
...
SQL;
            ...
            else
            {
				...
				INSERT INTO  users 
						(
							role_id, first_name, last_name, email, password, description, user_image, gender, 
							lang, is_active, created_at, updated_at,added_by
						)
				 VALUES (
                            '$iRoleId', '$sFirstName', '$sLastName', '$sEmail', '$spassword',"$sDescription", '$sUserImage', '$sGender', 
                            '$slang', '$iIs_active', '$dDate', '$dDate','$LoggedInUser' )
...
SQL;
    ...
    public function getAllTrainers($aParams = array()) {
        $aParams['roleId'] = ROLE_ID_TRAINER;

        return $this->getAllUsers($aParams);
    }

    public function getAllVolunteers($aParams = array()) {
        $aParams['roleId'] = ROLE_ID_VOLUNTEER;
        return $this->getAllUsers($aParams);
    }

    public function getAllTrainees($aParams = array()) {
        $aParams['roleId'] = ROLE_ID_TRAINEES;
        return $this->getAllUsers($aParams);
    }

    public function getAllCompanyRep($aParams = array()) {
        $aParams['roleId'] = ROLE_ID_COMPANY_REP;
        return $this->getAllUsers($aParams);
    }

    public function getAllEntrepreneur($aParams = array()) {
        $aParams['roleId'] = ROLE_ID_ENTREPRENEUR;
        return $this->getAllUsers($aParams);
    }
    ...
    /*
	 ...
	 /*
     * Get User By User Avaibility
     */
	public function getVolunteerByUserAvaibility($aParams)
	{
		 $iRoleId = ROLE_ID_VOLUNTEER;
		 $aAnd = '';
		  // Gender!

        if (isset($aParams['gender']) && $aParams['gender']) {

            $aAnd = " AND (u.gender = '" . $aParams['gender'] . "' ) ";
        }
		 
		 
		 $SQL = <<<SQL
                SELECT   DISTINCT u.user_id , u.first_name, u.last_name, u.email  
				FROM     users u , user_availability ua
				WHERE    u.user_id = ua.user_id
				AND		 u.role_id = '$iRoleId'
				$aAnd
...
SQL;
    ...
    public function getAllUsers($aParams) { //-->, $RoleId = 0, $iLimit = 0,$ReturnCount = FALSE,$Offset = 0,$Search = false
        ...
        if ($returnCount) {
            ...
            $sSelect = ' u.user_id, u.first_name, u.last_name, u.email ';

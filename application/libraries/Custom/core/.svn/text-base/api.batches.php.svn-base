...
class ApiBatches
{
	...
	function __construct($Data = array())
    {
        ...
        $this->CI->load->model('list_model',            'list');
    }
    ...
    function UploadBatchContentImage($sCallFrom ='',$aGivenData = array())
    {
        ...
        if($aData)
        {
            if($aGivenData['aUploadResult']['status'])
            {
                ...
                $CI = & get_instance();
                $CI->load->model('batches_model','batch');

                #Check for previous Entry
                $isEditMode =   false;
                $Exists     =   $CI->batch->CheckBatchElementData($aData);

                if($Exists) {$isEditMode = true;} //TODO : Remove thumbnails from batches in case of update

                $iSuccess   =   $CI->batch->UploadBatchContent(__FUNCTION__,array('aData' => $aData,'isEditMode' => $isEditMode) );
...
HTML;
    ...
    function setBatchTemplate($aData = array())
    {
        ...
        if($aData)
        {
            ...
            #In case of no error .. Save Data..!
            $iSettleTemplate     =   false;
            ###########################################################################################################
            #Check for previous Entry
            $iOldTemplateId     =   $this->CI->batch->getBatchTemplateId(__FUNCTION__,$aData);

            if($iOldTemplateId != $iTemplateId)
            {
                # Delete Old Template Data when user changed entire template
                $this->CI->batch->deleteOldBatchContent(__FUNCTION__,$iCampaignBatchId);
            }
            ...
            if($iSuccess)
            {
                ...
                {
                    $aFieldHtml                 =   $this->getFoldElementsHTML($aTemplateFoldsData,$iTemplateId,$iCampaignBatchId);
